...
我们致力于构建开放、共享的数学建模社区。以下是为美赛（MCM/ICM）备战推荐的优质开源项目与资源：

### 🛠️ 必备工具与模板

* **[LaTeX-in-ICM-MCM](https://github.com/QTH1225/LaTeX-in-ICM-MCM)**
    * **简介**：一个专为美赛设计的简单、全面且深度优化的 $\LaTeX$ 模板库。它不仅提供了开箱即用的论文写作解决方案（包含论文、Memo、海报等模板），还在 README 中整理了海量的建模资源（涵盖绘图、数据查找、AI 辅助等工具）。
    * **推荐理由**：如果你正在寻找一个规范的写作环境或是一站式的备赛资源索引，这个项目绝对不容错过。

> 🤝 **欢迎互链**：
> 如果你也有关于 **美赛/数学建模** 的优质开源项目（无论是优秀论文代码、通用模板、还是资源合集），欢迎通过 **Issue** 联系我或直接提交 **Pull Request** 进行友情链接互挂。让我们共同丰富开源社区，帮助更多的建模学子！

## ⚖️ 许可证
本项目采用 **MIT 许可证** 发行。
这意味着你可以自由地使用、复制、修改、合并、出版发行、散布、再授权及贩售本软件及其副本。
**唯一要求**：在软件的所有副本或重要部分中，必须包含上述版权声明和本许可声明（即注明来源）。
